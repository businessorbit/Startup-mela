...
const VerifyTicketPage = () => {
  ...
  useEffect(() => {
    ...
    // Handler for check-in events from other admins
    const handleCheckIn = (data) => {
      console.log("✓ Other admin checked in:", data);
      setLiveCheckedInCount((prev) => prev + 1);
    };

    // Subscribe to check-in events directly on socket instance
    socket.on("ticket:checked-in", handleCheckIn);
    console.log(
      "👂 Subscribed to 'ticket:checked-in' event on VerifyTicketPage",
    );
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-neutral-600">
              Scan or enter the 9-digit verification code
            </p>
          </div>
          {liveCheckedInCount > 0 && (
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-neutral-100">
            <form onSubmit={handleVerify} className="space-y-6">
              ...
              <button
                ...
                className="w-full py-4 rounded-xl bg-linear-to-r from-[#00C2FF] via-[#0070FF] to-[#00E29B] text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                    ...
                    <div className="bg-white rounded-xl p-4 space-y-2">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-black">
                            {result.ticket.itemType === "pass"
                              ? result.ticket.passType
                              : result.ticket.stallType}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-neutral-500 uppercase">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-neutral-700">
                            {new Date(result.ticket.checkInTime).toLocaleString(
                              "en-IN",
                            )}
                          </p>
                        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>
                    • Enter the 9-digit alphanumeric code from the attendee's
                    ticket
                  </li>
                  <li>• Each code can only be used once for check-in</li>
                  <li>
                    • Successful verification will mark the ticket as checked-in
                  </li>
                </ul>
              </div>
            </div>
